...
/**
 * {@link JdbcXaSinkFunction} tests using H2 DB.
 * H2 uses MVCC (so we can e.g. count records while transaction is not yet committed). But XA support isn't full, so
 * for some scenarios {@link org.apache.flink.api.java.io.jdbc.xa.h2.H2XaDsWrapper wrapper} is used, and for some - Derby.
 */
public class JdbcXaSinkH2Test extends JdbcXaSinkTestBase {
	...
	public void testHappyFlow() throws Exception {
		...
		assertEquals("record should not be inserted before the checkpoint started.", 0, xaHelper.countInDb());

		sinkHelper.snapshotState(Long.MAX_VALUE);
		assertEquals("record should not be inserted before the checkpoint completed.", 0, xaHelper.countInDb());

		sinkHelper.notifyCheckpointComplete(Long.MAX_VALUE);
		assertEquals("record should be inserted after the checkpoint completed.", 1, xaHelper.countInDb());
	}
