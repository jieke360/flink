...
/**
 * {@link JdbcXaSinkFunction} tests using Derby DB.
 * Derby supports XA but doesn't use MVCC, so we can't check anything before all transactions are completed.
 */
public class JdbcXaSinkDerbyTest extends JdbcXaSinkTestBase {

	/**
	 * checkpoint > capture state > emit > snapshot > close > init(captured state), open > emit > checkpoint.
	 */
	@Test
	...
	public void testTxEndedOnClose() throws Exception {
		sinkHelper.emit(TEST_DATA[0]); // don't snapshotState to prevent transaction from being prepared
		sinkHelper.close();
	...
	public void testTxRollbackOnStartup() throws Exception {
		...
		sinkHelper.close();
		xaHelper.assertPreparedTxCountEquals(1);
		TestXaSinkStateHandler state = new TestXaSinkStateHandler(); // forget about prepared tx, so it will not commit them and recover and rollback instead
	...
	public void testTwoCheckpointsComplete2nd() throws Exception {
		sinkHelper.emitAndSnapshot(JdbcTestFixture.CP0);
		sinkHelper.emitAndCheckpoint(JdbcTestFixture.CP1);
		xaHelper.assertDbContentsEquals(JdbcTestFixture.CP0, JdbcTestFixture.CP1); //		"both records should be inserted after the last snapshot completed."
	}
	...
	public void testFailurePropagation() throws Exception {
		...
		sinkHelper = new JdbcXaSinkTestHelper(buildAndInit(Integer.MAX_VALUE, XaFacadeImpl.fromXaDataSource(xaDataSource)), new TestXaSinkStateHandler());
