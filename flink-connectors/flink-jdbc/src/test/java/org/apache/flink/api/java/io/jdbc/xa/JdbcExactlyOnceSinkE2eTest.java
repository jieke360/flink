...
import org.apache.flink.api.java.io.jdbc.FlinkJdbcFacade;
...
public class JdbcExactlyOnceSinkE2eTest extends JdbcXaSinkTestBase {
	...
	private static final Supplier<XADataSource> DATA_SOURCE_SUPPLIER = new DerbyXaDataSourceSupplier();
	...
	public void test1() throws Exception {
		...
		env
				.addSource(new CheckpointAwaitingSource<>(TEST_DATA))
				.returns(TestEntry.class)
				.addSink(FlinkJdbcFacade.exactlyOnceSink(
						JdbcInsertOptions.builder().withQuery(String.format(INSERT_TEMPLATE, INPUT_TABLE)).build(),
						JdbcBatchOptions.builder().build(),
						JdbcExactlyOnceOptions.defaults(),
						DATA_SOURCE_SUPPLIER,
						ENTRY_TO_ROW_CONVERTER));
		env.execute();
	...
	protected XaSinkStateHandler createStateHandler() {
		return new XaSinkStateHandlerImpl();
	}

	@Override
	protected XaFacadeImpl getXaFacade() {
		return new XaFacadeImpl(DATA_SOURCE_SUPPLIER, Optional.empty());
	}

	/**
	 * {@link SourceFunction} emits all the data and waits for the checkpoint.
	 */
	private static class CheckpointAwaitingSource<T extends Serializable> implements SourceFunction<T>, CheckpointListener, CheckpointedFunction {
	...
	private static class DerbyXaDataSourceSupplier implements Supplier<XADataSource>, Serializable {
		@Override
		public XADataSource get() {
			return DERBY_EBOOKSHOP_DB.buildXaDataSource();
		}
	}
}
