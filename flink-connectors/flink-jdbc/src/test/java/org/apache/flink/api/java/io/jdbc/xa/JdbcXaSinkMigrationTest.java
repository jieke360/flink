...
import org.apache.flink.api.java.io.jdbc.DbMetadata;
...
import static org.apache.flink.api.java.io.jdbc.JdbcTestFixture.CP0;
...
import static org.apache.flink.api.java.io.jdbc.xa.JdbcXaSinkDerbyTest.derbyXaDs;
import static org.apache.flink.api.java.io.jdbc.xa.JdbcXaSinkTestBase.buildInitCtx;
...
public class JdbcXaSinkMigrationTest extends JdbcTestBase {
	...
	// or
	// mvn exec:java -Dexec.mainClass="<CLASS_NAME>" -Dexec.args='<VERSION>' -Dexec.classpathScope=test -Dexec.cleanupDaemonThreads=false
...
//		return Collections.singleton(MigrationVersion.v1_10);
	...
	public void testCommitFromSnapshot() throws Exception {
		...
		try (OneInputStreamOperatorTestHarness<TestEntry, Object> harness = createHarness(buildSink())) {
			harness.initializeState(readStateHandle(getSnapshotPath(readVersion)));
			harness.open();
		}
		try (JdbcXaFacadeTestHelper h = new JdbcXaFacadeTestHelper(JdbcXaSinkDerbyTest.derbyXaDs(), getDbMetadata().getUrl(), JdbcTestFixture.INPUT_TABLE)) {
	...
	private void preparePendingTransaction() throws Exception {
		try (JdbcXaSinkTestHelper sinkHelper = new JdbcXaSinkTestHelper(buildSink(), new XaSinkStateHandlerImpl())) {
	...
	private static OperatorSubtaskState captureState() throws Exception {
		try (JdbcXaSinkTestHelper sinkHelper = new JdbcXaSinkTestHelper(buildSink(), new XaSinkStateHandlerImpl())) {
			try (OneInputStreamOperatorTestHarness<TestEntry, Object> harness = createHarness(sinkHelper.getSinkFunction())) {
	...
	private static String getSnapshotPath(MigrationVersion version) {
		return String.format("src/test/resources/jdbc-exactly-once-sink-migration-%s-snapshot", version);
	}

	private static OneInputStreamOperatorTestHarness<TestEntry, Object> createHarness(JdbcXaSinkFunction<TestEntry> sink) throws Exception {
		OneInputStreamOperatorTestHarness<TestEntry, Object> harness = new OneInputStreamOperatorTestHarness<>(new StreamSink<>(sink));
		harness.setup();
		return harness;
	}
	...
	private static MigrationVersion parseVersionArg(String[] args) {
		return (args == null || args.length == 0 ? Optional.<String>empty() : of(args[0]))
				...
				.orElseThrow(() -> new IllegalArgumentException("Please specify a version as a 1st parameter. Valid values are: " + Arrays.toString(MigrationVersion.values())));
	}

	private static JdbcXaSinkFunction<TestEntry> buildSink() {
		return JdbcXaSinkTestBase.buildSink(getXidGenerator(), XaFacadeImpl.fromXaDataSource(derbyXaDs()), new XaSinkStateHandlerImpl(new XaSinkStateSerializer()), 1);
	}

	private static void cancelAllTx() throws Exception {
		try (JdbcXaFacadeTestHelper xa = new JdbcXaFacadeTestHelper(derbyXaDs(), JdbcTestFixture.DERBY_EBOOKSHOP_DB.getUrl(), JdbcTestFixture.INPUT_TABLE)) {
			xa.cancelAllTx();
		}
	}
	...
	private static void writeSnapshot(MigrationVersion v) throws Exception {
		...
		Preconditions.checkArgument(!Files.exists(Paths.get(path)), String.format("snapshot for version %s already exist: %s", v, path));
