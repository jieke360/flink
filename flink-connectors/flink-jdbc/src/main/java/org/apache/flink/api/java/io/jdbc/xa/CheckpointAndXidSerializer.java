...
final class CheckpointAndXidSerializer extends TypeSerializer<CheckpointAndXid> {
	...
	private static final TypeSerializerSnapshot<CheckpointAndXid> SNAPSHOT = new SimpleTypeSerializerSnapshot<CheckpointAndXid>(CheckpointAndXidSerializer::new) {
	...
	public CheckpointAndXid copy(CheckpointAndXid from) {
		return CheckpointAndXid.createRestored(from.checkpointId, from.attempts, xidSerializer.copy(from.xid));
	}
	...
	public CheckpointAndXid deserialize(DataInputView source) throws IOException {
		return CheckpointAndXid.createRestored(source.readLong(), source.readInt(), xidSerializer.deserialize(source));
	}
